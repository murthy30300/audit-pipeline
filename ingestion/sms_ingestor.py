...
class SMSIngestor(BaseIngestor):
    def load_csv(self):
    ...
    def validate(self, df):
        errors = []
        required_columns = ["message_id", "customer_id", "channel", "sent_time", "status"]
        ...
        duplicate_count = int(df["message_id"].duplicated().sum()) if "message_id" in df.columns else 0
