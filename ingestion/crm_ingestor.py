...
class CRMIngestor(BaseIngestor):
    def load_csv(self):
    ...
    def validate(self, df):
        errors = []
        required_columns = ["customer_id", "name", "phone_number"]
        ...
        duplicate_count = int(df["customer_id"].duplicated().sum()) if "customer_id" in df.columns else 0
